...
import pt.unl.fct.di.apdc.firstwebapp.util.ChangeUserStateData;
...
public class ChangeStateResource {
	...
	public Response changeUserState(ChangeUserStateData data, @CookieParam("session::apdc") Cookie cookie) { // ATENÇÃO
																												// AQUI
		// ESTOU A USAR
		// REGISTAR DATA
		// // DEPOIS // MUDAR
		LOG.fine("Attempt to change user state: " + data.username);
		...
		try {
			...
			if (user != null) {
				...
				if (checkPermissions(cookie, curRole)) {
					Entity.Builder userB = Entity.newBuilder(user); 
					userB.set("state", data.newstate);
					...
					txn.rollback();
					return Response.status(Status.FORBIDDEN).entity("You don't have permissions to change states.")
							.build();
				}
			} else {
				txn.rollback();
	...
	public static boolean checkPermissions(Cookie cookie, String role) {
		...
		if (!signatureNew.equals(signatureOld) || values[2].equals(USER) || (values[2].equals(GA) && role.equals(GA))
